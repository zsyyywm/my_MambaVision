# 方案1：DataC，512 + argmax（与 ``mamba_vision_tiny_datab_512x512_wire_iou.py`` 语义一致）。
# 用法：``python tools/train.py configs/mamba_vision/mamba_vision_tiny_datac_512x512_wire_iou.py``
# 数据根目录可用环境变量 ``WIRE_SEG_DATAC_ROOT`` 覆盖；默认 ``../../../DataC/DataC``（自
# ``semantic_segmentation`` 上至 ``autodl-tmp/DataC/DataC``，与 ``DataA-B`` 同级别目录并列）。
...
default_hooks = dict(
    ...
    checkpoint=dict(
        ...
        out_dir='checkpoint',
        filename_tmpl=(
            f'mav_t_datac512_s1_argmax_t{_wire_ts}_epoch{{}}.pth')),
